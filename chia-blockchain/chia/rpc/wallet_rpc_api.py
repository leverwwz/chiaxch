...
class WalletRpcApi:
    ...
    def get_routes(self) -> Dict[str, Callable]:
        return {
            ...
            "/get_transactions_by_height": self.get_transactions_by_height,
            "/de_puzzle_hash": self.de_puzzle_hash,
            "/en_puzzle_hash": self.en_puzzle_hash,
    ...
    async def add_key(self, request):
        ...
        if request_type == "new_wallet":
            started = await elf.service._start(fingerprint=fingerprint, new_wallet=True)
    ...
    async def get_transactions_by_height(self, request: Dict) -> Dict:
        assert self.service.wallet_state_manager is not None
        ...
        transactions = await self.service.wallet_state_manager.tx_store.get_transaction_between_height(wallet_id, start, end)
        formatted_transactions = []
        selected = self.service.config["selected_network"]
        prefix = self.service.config["network_overrides"]["config"][selected]["address_prefix"]
        for tx in transactions:
    ...
    async def de_puzzle_hash(self, request):
        """
        Returns a address
        """
        assert self.service.wallet_state_manager is not None

        puzzle_hash: bytes32 = decode_puzzle_hash(request["puzzle_hash"])

        return {
            "address": puzzle_hash.hex(),
        }

    async def en_puzzle_hash(self, request):
        """
        Returns puzzle_hash
        """
        assert self.service.wallet_state_manager is not None

        addr: str = encode_puzzle_hash(bytes.fromhex(request["address"]), "xch")
